<template>
  <div>
    ...
    <div v-if="showPilihKuota && !csvMode" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-md">
        <div class="bg-purple-600 text-white p-4 rounded-t-xl flex justify-between items-center">
          ...
          <button @click="$emit('close-pilih-kuota')" class="text-white hover:bg-purple-700 p-2 rounded-lg">✕</button>
        </div>

        <div class="p-6">
          <div v-if="loadingKuota" class="text-center py-8">
          ...
          <div class="flex gap-3 mt-6">
            <button @click="$emit('close-pilih-kuota')" class="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-800 py-2 rounded-lg">Batal</button>
            <button 
              @click="$emit('lanjut-form')"
    ...
    <div v-if="showPilihKuota && csvMode" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-md">
        <div class="bg-blue-600 text-white p-4 rounded-t-xl flex justify-between items-center">
          ...
          <button @click="$emit('close-pilih-kuota')" class="text-white hover:bg-blue-700 p-2 rounded-lg">✕</button>
        </div>
    ...
    <div v-if="showCsvUpload" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        <div class="bg-blue-600 text-white p-4 flex justify-between items-center">
          ...
          <button @click="$emit('close-csv-upload')" class="text-white hover:bg-blue-700 p-2 rounded-lg">✕</button>
        </div>
        ...
        <div class="p-6 overflow-y-auto flex-1">
          <div v-if="!csvFile" class="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:border-blue-500 transition-colors">
            <input 
              type="file" 
              accept=".csv" 
              @change="$emit('csv-file-change', $event)"
              class="hidden" 
              id="csvInput"
            >
    ...
    <div v-if="csvResults.total > 0" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        >
          ...
          <button @click="$emit('close-csv-results')" class="text-white hover:bg-black/20 p-2 rounded-lg">✕</button>
        </div>
    ...
    <div v-if="editItem" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div class="bg-yellow-500 text-white p-4 rounded-t-xl flex justify-between items-center">
          ...
          <button @click="$emit('close-edit')" class="text-white hover:bg-yellow-600 p-2 rounded-lg">✕</button>
        </div>
        ...
        <div class="p-6 space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input 
                :value="editForm.nama_pemilik_atm"
                @input="updateEditForm('nama_pemilik_atm', $event.target.value)"
                type="text"
            ...
            <div>
              ...
              <input 
                ...
                @input="updateEditForm('email', $event.target.value)"
                type="email"
                class="w-full border rounded-lg px-3 py-2"
              >
            </div>
            ...
            <div>
              ...
              <input 
                ...
                @input="updateEditForm('whatsapp', $event.target.value)"
                type="text"
                class="w-full border rounded-lg px-3 py-2"
            ...
            <div>
              ...
              <input 
                :value="editForm.nomor_kk"
                @input="updateEditForm('nomor_kk', $event.target.value)"
                type="text"
                maxlength="16"
            ...
            <div>
              ...
              <input 
                :value="editForm.nomor_atm"
                @input="updateEditForm('nomor_atm', $event.target.value)"
                type="text"
                maxlength="18"
                class="w-full border rounded-lg px-3 py-2 font-mono"
            ...
            <div>
              ...
              <select 
                :value="editForm.kartu_pemanfaat"
                @change="updateEditForm('kartu_pemanfaat', $event.target.value)"
            ...
            <div>
              ...
              <input 
                ...
                @input="updateEditForm('kelurahan', $event.target.value)"
                type="text"
                class="w-full border rounded-lg px-3 py-2"
            ...
            <div>
              ...
              <input 
                :value="editForm.rt"
                @input="updateEditForm('rt', $event.target.value)"
                type="text"
                maxlength="3"
                class="w-full border rounded-lg px-3 py-2"
            ...
            <div>
              ...
              <input 
                :value="editForm.rw"
                @input="updateEditForm('rw', $event.target.value)"
                type="text"
                maxlength="3"
                class="w-full border rounded-lg px-3 py-2"
          ...
          <div>
            ...
            <textarea 
              ...
              @input="updateEditForm('alamat', $event.target.value)"
              rows="3"
              class="w-full border rounded-lg px-3 py-2"
    ...
    <div v-if="detailItem" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div class="p-6 space-y-6">
          ...
          <div class="flex gap-3 pt-4 border-t">
            <button @click="$emit('detail-download-qr', detailItem)" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-lg font-medium flex items-center justify-center gap-2">📥 Download QR</button>
            
            <button 
              @click="$emit('detail-open-wa', detailItem); $emit('close-detail')"
            ...
            <template v-if="canEdit && detailItem.status === 'menunggu'">
              <button @click="$emit('detail-verifikasi', detailItem.id)" class="flex-1 bg-green-600 hover:bg-green-700 text-white py-2 rounded-lg font-medium">✓ Verifikasi</button>
              <button @click="$emit('detail-tolak', detailItem)" class="flex-1 bg-red-600 hover:bg-red-700 text-white py-2 rounded-lg font-medium">✕ Tolak</button>
            </template>
    ...
    <!-- Modal: Tolak -->
    <div v-if="showTolakItem && canEdit" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl shadow-xl p-6 w-full max-w-md">
        ...
        <div class="mb-4">
          ...
          <select 
            :value="alasanTolak"
            @change="$emit('update-alasan-tolak', $event.target.value)"
            class="w-full border rounded-lg px-3 py-2 bg-white"
          >
          ...
          <textarea 
            ...
            :value="alasanLainnya"
            @input="$emit('update-alasan-lainnya', $event.target.value)"
            rows="2" 
        ...
        <div class="flex gap-3">
          <button @click="$emit('close-tolak')" class="flex-1 bg-gray-200 hover:bg-gray-300 py-2 rounded-lg">Batal</button>
          <button 
            @click="$emit('submit-tolak')" 
...
const props = defineProps({
  showPilihKuota: Boolean,
  csvMode: Boolean,
  kuotaList: Array,
  loadingKuota: Boolean,
  selectedKuota: Object,
...
const emit = defineEmits([
  'close-pilih-kuota', 'pilih-kuota', 'lanjut-form', 'lanjut-csv',
  'close-csv-upload', 'close-csv-results', 'csv-file-change', 'csv-ganti-file',
  'submit-csv', 'download-csv-report',
  'close-detail', 'detail-download-qr', 'detail-open-wa', 'detail-verifikasi', 'detail-tolak',
  'close-edit', 'submit-edit', 'update-edit-form',
  'update-alasan-tolak', 'update-alasan-lainnya', 'close-tolak', 'submit-tolak'
])
...
const updateEditForm = (field, value) => {
  emit('update-edit-form', { ...props.editForm, [field]: value })
}
</script>