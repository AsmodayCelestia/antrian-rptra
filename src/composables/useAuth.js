...
export const isAdmin = ref(initialSession?.user?.role === 'admin' || false)
export const isModerator = ref(initialSession?.user?.role === 'moderator' || false)
...
export const login = async (email, password) => {
  ...
  try {
    ...
    // ⭐ PENTING: Update refs dengan nextTick untuk ensure reactivity
    session.value = newSession
    user.value = newSession.user
    isAdmin.value = admin.role === 'admin'
    isModerator.value = admin.role === 'moderator'
    
    await nextTick() // Ensure Vue update DOM
